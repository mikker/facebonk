...
import { defaultStorageDir } from './storage-path.js'
...
export function formatRuntimeError(error, baseDir) {
...
export async function main(argv = getArgv()) {
  const { baseDir, args } = parseArgs(argv)
...
export function runCli(argv = getArgv()) {
  return main(argv).catch((error) => {
    const { baseDir } = parseArgs(argv)
